...
result_path="/analysis/lots_for_item_indexed/$1_$2"
user=$POSTGRES_USER
db=$POSTGRES_DB
...
for (( attempt = 1; attempt <= attempts; ++attempt )); do
    psql -U $user -d $db -c "EXPLAIN ANALYZE SELECT * FROM lots WHERE item_id = $item_id AND game_id = $game_id;" > "$result_path/$attempt"
